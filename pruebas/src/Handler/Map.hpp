...
struct Map
{
    ...
    void draw_layer(tmx_map *m, tmx_layer *l) {
        unsigned long i, j;
        ...
        for (i=0; i<map->height; i++) {
            for (j=0; j<map->width; j++) {
                gid = (l->content.gids[(i*map->width)+j]) & TMX_FLIP_BITS_REMOVAL;
                if (map->tiles[gid] != NULL) {
                    ts = m->tiles[gid]->tileset;
                    im = m->tiles[gid]->image;
                    x  = m->tiles[gid]->ul_x;
                    y  = m->tiles[gid]->ul_y;
                    w  = ts->tile_width;
                    h  = ts->tile_height;
                    ...
                    flags = (l->content.gids[(i*map->width)+j]) & ~TMX_FLIP_BITS_REMOVAL;
    ...
    void DrawAll(){
        ...
        if(map->ly_head != nullptr){
            printf("No es nullptr\n");
